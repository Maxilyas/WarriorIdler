...
const MAX_CODE = 60_000   // anti-bloat : le catalogue est bundlé dans l'app ; un build réel ≈ 8-24 ko
const MAX_BUILDS = 5_000  // garde-fou anti-spam global du catalogue
const body = process.env.ISSUE_BODY || ''
const title = process.env.ISSUE_TITLE || ''
const author = (process.env.ISSUE_AUTHOR || '').replace(/[^A-Za-z0-9_-]/g, '').slice(0, 39) // login GitHub = [a-z0-9-]
...
// 1) Extraire le code WIB1: (robuste quel que soit le formatage de l'issue). Plafond de TAILLE.
const m = body.match(/WIB1:[A-Za-z0-9+/=]+/)
if (!m) fail('Aucun code « WIB1:… » trouvé dans la soumission.')
const code = m[0]
if (code.length > MAX_CODE) fail(`Code trop volumineux (${code.length} > ${MAX_CODE} caractères).`)

// 2) Décoder + valider structurellement (pas de dépendance au moteur ici — JSON.parse n'exécute rien).
let cfg
...
// retire les caractères dangereux pour le shell (commit), le JS (commentaire) ET le HTML (leaderboard).
...
// 4) Ajouter au catalogue (anti-doublon par code, garde-fou de volume).
const list = JSON.parse(fs.readFileSync(FILE, 'utf-8'))
if (list.length >= MAX_BUILDS) fail('Catalogue plein — soumission refusée pour le moment.')
