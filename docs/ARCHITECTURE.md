...
| `store.ts` | **Assembleur du store Zustand** (~500 lignes) : types, état initial (`create()` : loadSave + hors-ligne + cold-start) et **spread des 10 slices d'actions**. Plus de logique de jeu inline. | [10](systemes/10-etat-store-et-sauvegarde.md) |
| `*Slice.ts` (×10) | **Slices d'actions par domaine** : `tickSlice` (boucle de combat), `worldSlice`, `stuffSlice`, `gemsSlice`, `officineSlice`, `atelierSlice`, `expeditionsSlice`, `herosSlice`, `liveopsSlice`, `marketSlice`. Chacun : `createXSlice(set, get)` → `Pick<GameState, …>`. `sliceTypes.ts` = `GameSet`/`GameGet`. | [10](systemes/10-etat-store-et-sauvegarde.md) |
...
| `storeHelpers.ts` | **Helpers du store** : fonctions pures d'orchestration + consts partagés (journal, XP métiers/Conseil, coffres, prix échoppe, `tickDungeon`/`tickRaid`/`applyChestRewards`, `awaySince`) — extrait de `store.ts` | [10](systemes/10-etat-store-et-sauvegarde.md) |
